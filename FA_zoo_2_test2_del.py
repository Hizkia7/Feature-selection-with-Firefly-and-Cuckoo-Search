...
######################  Import Dataset  ######################
##############################################################
header=['animal_name','hair','feathers','eggs','milk',
        'airbone','aquatic','predator','toothed','backbone',
        'breathes','venomous','fins','legs','tail','domestic',
        'catsize','class']

df_zoo=pd.read_csv('dataset\\zoo.data', header=None)
df_zoo.columns=header
# Check data type
print(df_zoo)
df_zoo.dtypes    
#avoid missing data
df_zoo=df_zoo.dropna()
#encode
le=LabelEncoder()
df_zoo['class']=le.fit_transform(df_zoo['class'])

x_zoo=df_zoo.drop(['animal_name','class'], axis=1)
y_zoo=df_zoo['class']
...
def move_Xj_towards_Xk(a,βo, encode1, encode2):
    ...
    #β = βo*math.exp(γrjk)
    β = βo*np.exp(γrjk)
    new_firefly = np.array(encode1)+np.array(β*(np.array(encode2)-np.array(encode1)))+np.array(a*np.array(np.random.uniform(low = -0.5, high = 0.5, size=(x_zoo.shape[1]))))
...
def particle_best(feature_index_list, encode_list, encode_binary_list):   #一開始全部未經select的feature放進去MLP求accuracy
    ...
    for i in range(len(feature_index_list)):
        x = x_zoo.iloc[:, feature_index_list[i]]
        y = y_zoo
        # labelencoder_Y = LabelEncoder()
        # y = labelencoder_Y.fit_transform(y)
        x_train, x_test, y_train, y_test = train_test_split1(x, y)
        x_train, x_test = standard_scaler(x_train, x_test)
        model, history, max_acc = NN_model_structure_classification(x_train, y_train, 20, 50, x_test, y_test)
...
def twopointchange(new_firefly, new_firefly_binary, num_of_features):
    ...
    position = np.random.choice(num_of_features, 2, replace = False)
...
def shaking(new_firefly, new_firefly_binary, num_of_features):
  ...
  times=round(num_of_features*k)  

  for i in range(times):
      position_shaking = np.random.randint(0,num_of_features)
      for k in range(len(new_firefly_binary)):
                if new_firefly_binary[position_shaking] == 1:
                  new_firefly_binary[position_shaking] = 0
                  new_firefly[position_shaking]=np.random.uniform(0,0.5)
                  check = np.sum(new_firefly_binary)
                  while check == 0:
                      position = np.random.randint(0,num_of_features)
                      new_firefly_binary[position]= 1
...
# #global best
        ...
        for j in range(length):
            ...
            for l in range(5):
                ...
                encode_VNS, encode_VNS_binary = shaking(old_VNS_encode.copy(), old_VNS_encode_binary.copy(), 16)
          ...
          #new:  
                ...
                for m in range(2):
                    ...
                    for n in range(10): 
                        if m==0:
                              encode_VNS, encode_VNS_binary = onepointchange(old_VNS_encode.copy(), old_VNS_encode_binary.copy(), 16)
                              #print('VNS1_encode:',encode_VNS)
                              #print('VNS1_encode_binary:',encode_VNS_binary)
                        else:
                              encode_VNS, encode_VNS_binary = twopointchange(old_VNS_encode.copy(), old_VNS_encode_binary.copy(), 16)
                        ...
                        #if VNS_result[0]>old_VNS_result[0] and VNS_result[1]>old_VNS_result[1]:
...
#                 #print('best_VNS_result_list[j]:',best_VNS_result_list[j][1])
                #if old_VNS_result[0] > best_VNS_result_list[j][0] and old_VNS_result[1] > best_VNS_result_list[j][1]:   
...
#(6)在哪邊有current_best,不要亂寫,那是best_vns_firefly_result_list[j][1],出了vns,要先把best_vns還回給原先的firefly解,
            ...
            #if firefly_result_list[p][0] > current_best_result[0] and firefly_result_list[p][1] > current_best_result[1]:
...
encode_list, encode_binary_list, feature_index_list= initial_sol(20, 16)
...
best_encode, best_binary, best_result, best_feature = FA_Algorithm(encode_list, encode_binary_list, firefly_result_list, feature_index_list, x_zoo, y_zoo, current_best_encode, current_best_binary, current_best_result, current_best_feature_subset)
...
f = open('zoo_vns_prof_lasttest_new(wz GPU_memory_setting)_iter_5_VNS_iter5_converge_10_BS_20_epochs_50_NP_20_RS_5_a_1_b_m2_g_0.1_threshold_0.5.txt', 'w')
