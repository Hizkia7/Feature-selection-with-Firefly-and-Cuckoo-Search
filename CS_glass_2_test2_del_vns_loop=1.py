...
header_list=['id','RI','Na','Mg','Al','Si','K','Ca','Ba','Fe','Class']

df_glass=pd.read_csv('dataset\\glass.data', header=None)
    
df_glass.columns=header_list
df_glass.dropna()
#checking data types
df_glass.dtypes
#encode class
le=LabelEncoder()
df_glass['Class']=le.fit_transform(df_glass['Class'])
df_glass_final=df_glass.drop(['Class'], axis=1)
...
def particle_best(encode_list, encode_binary_list, feature_subset_list):   #一開始全部未經select的feature放進去MLP求accuracy
    ...
    for i in range(len(feature_subset_list)):
        x = df_glass_final.iloc[:, feature_subset_list[i]]
        y = df_glass['Class']
        #y = to_categorical(y)
...
def fitness_of_new_nest(new_encode, new_encode_binary, new_feature_subset):
    ...
    x = df_glass_final.iloc[:, new_feature_subset]
    y = df_glass['Class']
    #y = to_categorical(y)
    x_train, x_test, y_train, y_test = train_test_split1(x, y)
    #x_train, x_test = standard_scaler(x_train, x_test)
...
def twopointchange(new_encode, new_encode_binary):
    ...
    #update encode and encode binary
    position = np.random.choice(10, 2, replace = False)
...
def CSA_Algorithm(encode_list, encode_binary_list, feature_subset_list, cuckoo_result_list, gbest, gbest_binary, gbest_feature_subset, gbest_result):
    ...
    for i in range(iteration):
        ...
        encode_list=sort[:,2:12]
        encode_binary_list=sort[:,12:22]
        feature_subset_list=sort[:,22:]
...
#EXECUTE
encode_list, encode_binary_list, feature_subset_list= initial_sol(20, 10)
...
f = open('glass_vns_prof_test_lasttest_new(wz GPU_memory_setting)_iter_5_VNS_iter10_converge_10_BS_1_epochs_10_NP_20_RS_20_a_1_b_m2_g_0.1_threshold_0.5.txt', 'w')
