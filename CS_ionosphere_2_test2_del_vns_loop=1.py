...
df_ionosphere=pd.read_csv('dataset\\ionosphere.csv')
# Check data type
print(df_ionosphere)
df_ionosphere.dtypes
    
#avoid missing data
df_ionosphere=df_ionosphere.dropna()
#encoding class
le=LabelEncoder()
df_ionosphere['35']=le.fit_transform(df_ionosphere['35'])
df_ionosphere['35']=df_ionosphere['35'].astype(np.int64)
# recheck data type
print(df_ionosphere)
df_ionosphere.dtypes


df_ionosphere_final=df_ionosphere.drop(['35'], axis=1)
...
def particle_best(encode_list, encode_binary_list, feature_subset_list):   #一開始全部未經select的feature放進去MLP求accuracy
    ...
    for i in range(len(feature_subset_list)):
        x = df_ionosphere_final.iloc[:, feature_subset_list[i]]
        y = df_ionosphere['35']
...
def fitness_of_new_nest(new_encode, new_encode_binary, new_feature_subset):
    ...
    x = df_ionosphere_final.iloc[:, new_feature_subset]
    y = df_ionosphere['35']
    x_train, x_test, y_train, y_test = train_test_split1(x, y)
    x_train, x_test = standard_scaler(x_train, x_test)
    model, history, max_acc = NN_model_structure_classification(x_train, y_train, 32, 50, x_test, y_test)
...
def twopointchange(new_encode, new_encode_binary):
    ...
    #update encode and encode binary
    position = np.random.choice(34, 2, replace = False)
...
def CSA_Algorithm(encode_list, encode_binary_list, feature_subset_list, cuckoo_result_list, gbest, gbest_binary, gbest_feature_subset, gbest_result):
    ...
    for i in range(iteration):
        ...
        encode_list=sort[:,2:36]
        encode_binary_list=sort[:,36:70]
        feature_subset_list=sort[:,70:]
...
#EXECUTE
encode_list, encode_binary_list, feature_subset_list= initial_sol(20, 34)
...
f = open('ionosphere_vns_prof_test_lasttest_new(wz GPU_memory_setting)_iter_5_VNS_iter10_converge_10_BS_1_epochs_10_NP_20_RS_20_a_1_b_m2_g_0.1_threshold_0.5.txt', 'w')
