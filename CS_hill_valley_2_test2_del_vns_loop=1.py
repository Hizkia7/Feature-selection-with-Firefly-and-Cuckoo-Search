...
df_hill=pd.read_csv('dataset\\Hill_Valley_without_noise.csv')
print(df_hill)
df_hill.dtypes
    
#avoid missing data
df_hill=df_hill.dropna()

df_hill_final=df_hill.drop(['class'], axis=1)
...
def particle_best(encode_list, encode_binary_list, feature_subset_list):   #一開始全部未經select的feature放進去MLP求accuracy
    ...
    for i in range(len(feature_subset_list)):
        x = df_hill_final.iloc[:, feature_subset_list[i]]
        y = df_hill['class']
...
def fitness_of_new_nest(new_encode, new_encode_binary, new_feature_subset):
    ...
    x = df_hill_final.iloc[:, new_feature_subset]
    y = df_hill['class']
    x_train, x_test, y_train, y_test = train_test_split1(x, y)
    x_train, x_test = standard_scaler(x_train, x_test)
    model, history, max_acc = NN_model_structure_classification(x_train, y_train, 32, 50, x_test, y_test)
...
def twopointchange(new_encode, new_encode_binary):
    ...
    #update encode and encode binary
    position = np.random.choice(100, 2, replace = False)
...
def CSA_Algorithm(encode_list, encode_binary_list, feature_subset_list, cuckoo_result_list, gbest, gbest_binary, gbest_feature_subset, gbest_result):
    ...
    for i in range(iteration):
        ...
        encode_list=sort[:,2:102]
        encode_binary_list=sort[:,102:202]
        feature_subset_list=sort[:,202:]
...
#EXECUTE
encode_list, encode_binary_list, feature_subset_list= initial_sol(20, 100)
...
f = open('hill_valley_vns_prof_test_lasttest_new(wz GPU_memory_setting)_iter_5_VNS_iter10_converge_10_BS_1_epochs_10_NP_20_RS_20_a_1_b_m2_g_0.1_threshold_0.5.txt', 'w')
